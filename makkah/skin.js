...
// Generated Mon Dec 23 16:54:42 2024
...
function pano2vrSkin(player,base) {
	...
	this.addSkin=function() {
		...
		me._map_1.callChildLogicBlocksHotspot_ht_node_changenodeid = function(){
			if(me._map_1.ggMarkerInstances) {
				var i;
				for(i = 0; i < me._map_1.ggMarkerInstances.length; i++) {
					if (me._map_1.ggMarkerInstances[i]._ht_node_image && me._map_1.ggMarkerInstances[i]._ht_node_image.logicBlock_scaling) {
						me._map_1.ggMarkerInstances[i]._ht_node_image.logicBlock_scaling();
					}
				}
			}
		}
		me._map_1.callChildLogicBlocksHotspot_ht_node_varchanged_ht_ani = function(){
			if(me._map_1.ggMarkerInstances) {
				var i;
				for(i = 0; i < me._map_1.ggMarkerInstances.length; i++) {
					if (me._map_1.ggMarkerInstances[i]._ht_node_image && me._map_1.ggMarkerInstances[i]._ht_node_image.logicBlock_scaling) {
		...
		me._map_1.ggInitMapMarkers=function(updateMapBounds) {
			...
			for(var i=0;i<ids.length;i++) {
				...
				if ((gps.length>=2) && ((gps[0]!=0) || (gps[1]!=0))) {
					...
					var div=new SkinElement_ht_node_Class(me, markerParent);
					marker=new L.SkinMarkerLayer(div._ht_node,markerLocation);
		...
		me._map_1.callChildLogicBlocksHotspot_ht_node_changenodeid();
		me._map_1.callChildLogicBlocksHotspot_ht_node_varchanged_ht_ani();
	...
	function SkinElement_ht_node_Class(parentScope,ggParent) {
		var me=this;
		var flag=false;
		me.parentScope=parentScope;
	...
	me.addSkin();
	player.addListener('changenodeid', function(args) { me._map_1.callChildLogicBlocksHotspot_ht_node_changenodeid(); });
	player.addListener('varchanged_ht_ani', function(args) { me._map_1.callChildLogicBlocksHotspot_ht_node_varchanged_ht_ani(); });
